...
if ($env:EVAL_OR_NOT -eq "false") {
    ...
    & torchrun --standalone --nproc_per_node=$env:NPROC_PER_NODE -m scripts.base_eval -- --device-batch-size=16
}
...
Write-Host ""
if ($env:EVAL_OR_NOT -eq "false") {
    Write-Host "Skipping sft as per EVAL_OR_NOT=$env:EVAL_OR_NOT"
    } else {
    & torchrun --standalone --nproc_per_node=$env:NPROC_PER_NODE -m scripts.chat_sft -- --device-batch-size=16 --run=$env:WANDB_RUN
}


Write-Host ""
Write-Host "    !!"
Write-Host "Evaluating supervised finetuning (SFT)..."
Write-Host "    !!"
Write-Host ""
if ($env:EVAL_OR_NOT -eq "false") {
